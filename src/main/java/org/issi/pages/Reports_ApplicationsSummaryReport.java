...
public class Reports_ApplicationsSummaryReport extends BasePage{

    protected Reports_ApplicationsSummaryReport() {
    }
    // clicking on Applicaitons menu item
    private static final By reportsModule = By.xpath("//a[contains(text(),'Reports')]");
    public static void clickOnReportsModule() {
        clickOn(reportsModule, WaitStrategy.CLICKABLE, "Reports Module");
        waitupto(1000);
    }

    private static final By applicationSummeryReport = By.xpath("//a[contains(text(),'Applications Summary Report')]");
    public static void clickOnApplicationSummeryReport() {
        clickOn(applicationSummeryReport, WaitStrategy.CLICKABLE, "Application Summery Report");
        waitupto(1000);
    }
    ...
    private static final By excelbtn = By.xpath("//input[@value='Excel']");
    public static void clickOnExcelButton() {
        List<WebElement> Excel= DriverManager.getDriver().findElements(By.xpath("//input[@value='Excel']"));

        for (int i=1; i<=Excel.size() ;i++) {
            DriverManager.getDriver().findElement(By.xpath("(//input[@value='Excel'])["+i+"]")).click();
           waitupto(1000);
        }
    }
